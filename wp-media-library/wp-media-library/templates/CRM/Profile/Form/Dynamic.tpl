        ...
        {if array_key_exists('options_per_line', $field) && $field.options_per_line}
          ...
          <div id="editrow-{$profileFieldName}" class="crm-section editrow_{$profileFieldName}-section form-item">
            ...
            <div class="edit-value content">
              ...
              <div id="name-{$profileFieldName}" name="name-{$profileFieldName}" class="" ></div>
              {*$imageURL|@print_r*}
              {if !empty($imageURL)}
              ...
              {else}
              <div id="crm-event-links-link" class="button button-{$profileFieldName}" >Ajouter Media</div></a>
              {/if}
              {literal}
              ...
              jQuery(document).ready( function( $ ) {
                ...
                $('#{/literal}{$profileFieldName}{literal}').addClass("hidden");
              
                {/literal}
                
                {if $formElement.value != ''}
                 {literal}
                    $('.img-{/literal}{$profileFieldName}{literal}').css("max-height","150px");
                  {/literal}
                {/if}
                {literal}
                
                $('.button-{/literal}{$profileFieldName}{literal}').click(function(e) {
                  callWpMediaUpload("#{/literal}{$profileFieldName}{literal}","");
                });

                $("#{/literal}{$profileFieldName}{literal}").change(function(e) {
                  if($("#{/literal}{$profileFieldName}{literal}").val() == ""){

                    $("#img-{/literal}{$profileFieldName}{literal}").css('backgournd-image','none');
                    $("#img-{/literal}{$profileFieldName}{literal}").css('max-width','unset');
                    $("#img-{/literal}{$profileFieldName}{literal}").html("Cliquez sur 'Enregistrer' pour supprimer l'image d&eacute;finitivement.");
                    $(".button-delete-{/literal}{$profileFieldName}{literal}").addClass("hidden");

                  }else{
                    const url = new URL($("#{/literal}{$profileFieldName}{literal}").val());
                        var name = url.searchParams.get('filename');
                        $("#name-{/literal}{$profileFieldName}{literal}").html(name);
              ...
              {else}
                ...
                {if $field.html_type neq 'File' || ($field.html_type eq 'File' && !$field.is_view)}
                  ...
                  {if $profileFieldName|substr:0:7 eq 'custom_'}
                  
                    {assign var="custom_id" value=$profileFieldName|replace:'custom_':''}
          ...
          {if $textpos > 0}
            ...
            <div id="" name="" class="" style="display:none;">
              ...
              <div id="name-{$profileFieldName}" name="name-{$profileFieldName}" class="" ></div>
              {if $form.$profileFieldName.value != ''}
                <div id="img-{$profileFieldName}" name="img-{$profileFieldName}" class="img-{$profileFieldName}" style="background-image:url('{$form.$profileFieldName.value}');background-size:cover;background-repeat:no-repeat;max-width:150px;">
                <img id='img-{$profileFieldName}-img' name='img-{$profileFieldName}-img' class='' src='{$form.$profileFieldName.value}'></br>
                </div>
                <span class="button-delete-{$profileFieldName}"><a class="action-item profile-hover-button small-popup" onclick="if (confirm(&quot;\u00cates-vous s\u00fbr de vouloir supprimer l'image.&quot;)) callWpMediaSuppress('#{$profileFieldName}'); else return false;">Supprimer l'image</a></span>
              {else}
              <div id="crm-event-links-link" class="button button-{$profileFieldName}" >Ajouter Media</div></a>
              {/if}
              {literal}
                ...
                {literal}
                  ...
                  $("#{/literal}{$profileFieldName}{literal}").change(function(e) {
                    ...
                    if($("#{/literal}{$profileFieldName}{literal}").val() == ""){
                      ...
                      $("#img-{/literal}{$profileFieldName}{literal}").css('background-image', 'none');
                      $("#img-{/literal}{$profileFieldName}{literal}").css('max-width','unset');
                      $("#img-{/literal}{$profileFieldName}{literal}").css('padding-top','5px');
                      $("#img-{/literal}{$profileFieldName}{literal}").css('padding-bottom','5px');
                      $("#img-{/literal}{$profileFieldName}{literal}").html("<strong>Cliquez sur 'Enregistrer' pour supprimer l'image d&eacute;finitivement.</strong>");
                      $(".button-delete-{/literal}{$profileFieldName}{literal}").addClass("hidden");
