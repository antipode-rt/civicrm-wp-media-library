...
<div class="crm-block crm-form-block crm-custom-field-form-block">
  <table class="form-layout">
    ...
    {/if}
        {* RT START *}
    <tr class="crm-custom-field-form-block-link-to-wp link_to_wp"  name="link_to_wp" id="link_to_wp" {if !( $action eq 1 || $action eq 2 ) && ($form.data_type.value.0.0 != 0)}class="hiddenElement"{/if}>
      <td class="label"><label>{ts}Link to WP Media Library{/ts}</label></td>

      {if $action eq 1 }
        <td class="html-adjust"><input id="link_to_wp" name="link_to_wp" type="checkbox" value="1" class="crm-form-checkbox"></td>
      {elseif  $action eq 2}
       {if ($smarty.get.id && $smarty.get.gid) }

       {crmAPI var='result' entity='CustomField' action='get' return="name" id=$smarty.get.id custom_group_id=$smarty.get.gid}
 
        {assign var="field_name" value=$result.values[0].name}

       {/if}
       {if $field_name|strpos:"_lwp" !== false }
        <td class="html-adjust">{ts}Oui{/ts}</td>
       {else}
        <td class="html-adjust">{ts}Non{/ts}</td>
